...
export default class SlackTabResponse extends Component {
          ...
          <div className="slack__response-attachment-body no-border">
            <div className="c-message__content_header">
              <span className="c-message__sender">
                <span className="slack__app-timestamp">
                  Last Updated | Today at {moment().format('h:mm a')}
                </span>
              </span>
              <div className="slack__response-attachment-text">
                <span className="slack__response-attachment-button basic">
                  ♻<span className="basic">Refresh</span>
                </span>
              </div>
            </div>
          </div>
  ...
  renderPrimaryButtons(buttonLabel) {
    const { moreActionsRef, selectedAction } = this.props
    ...
    if (!blankCanvas) {
      if (showStatusButtons) {
        return (
          <Fragment>
            <span className="slack__response-attachment-button basic">
              <span className="basic">No</span>
            </span>
            <span className="slack__response-attachment-button basic">
              <span className="basic">Maybe</span>
            </span>
            <span className="slack__response-attachment-button">
              You Accepted
            </span>
          </Fragment>
        )
      ...
      <Fragment>
        {buttonArray.map((button, i) => {
          return (
            <span
              key={i}
              className={cx('slack__response-attachment-button', {
                basic: button.type === 'basic',
                danger: button.type === 'danger',
              })}
            >
              <span className={cx(button.type)}>{button.text}</span>
            </span>
          )
        })}
      </Fragment>
    ...
    return (
      <Fragment>
        <div className="slack__label">
          {!blankCanvas && <EyeFilledIcon className="slack__label-icon" />}
          <span className={cx('slack__label-text', { blank: blankCanvas })}>
            Only visible to you
          </span>
        </div>
        <div className="slack__gutter">
          {!blankCanvas && <GonebusyAvatar className="slack__avatar-image" />}
        </div>
        <div
          className={cx('slack__content', { blank: blankCanvas })}
          data-qa="message_content"
        >
          <div className="c-message__content_header">
            <span className="c-message__sender">
              <span className={cx('slack__app-name', { blank: blankCanvas })}>
                Gonebusy
              </span>
              <span className={cx('slack__app-badge', { blank: blankCanvas })}>
                App
              </span>
              <span
                className={cx('slack__app-timestamp', { blank: blankCanvas })}
              >
          ...
          <div className="slack__response-attachments">
            {events.map((event, i) => (
              <div key={i} className="slack__response-attachment">
                <div
                  className={cx('slack__response-attachment-body', {
                    'no-border': blankCanvas || event.noAuthor,
                  })}
                >
                  <div className="slack__response-attachment-author">
                    {!blankCanvas && !event.noAuthor && (
                      <DefaultAvatar className="slack__response-attachment-author-icon" />
                    )}
                    <span
                      className={cx('slack__response-attachment-author-name', {
                        blank: blankCanvas,
                        hidden: event.noAuthor,
                      })}
                    >
                      Your Slack Name
                    </span>
                  </div>
                  <div className="slack__response-attachment-title">
                    {event.responseTitle}
                  </div>
                  <div className="slack__response-attachment-text">
                    {event.responseDescription}
                  </div>
                  <div className="slack__response-attachment-text">
                    {event.responseMembers && (
                      <SlackText
                        text={event.responseMembers}
                        blank={blankCanvas}
                      />
                    )}
                  </div>
                  {fullDescriptionVisible && (
                    <div className="slack__response-attachment-text-full-description">
                      {event.responseFullDescription}
                    </div>
                  )}
                  {event.showAddedToGoogleCalender && (
                    <div className="slack__response-attachment-footer">
                      ...
                      <span className="slack__response-attachment-footer-text">
                  ...
                  <div className="slack__response-attachment-text-actionrow">
                    ...
                    {event.buttonArray &&
                      this.renderButtonArray(event.buttonArray)}
                  </div>
                </div>
              </div>
            ))}
